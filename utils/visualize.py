...
import py3Dmol  # 导入 py3Dmol，用于 3D 可视化。
from rdkit import Chem  # 导入 RDKit Chem 模块。


def visualize_complex(pdb_block, sdf_block, show_protein_surface=True, show_ligand=True, show_ligand_surface=True):
    view = py3Dmol.view()  # 创建 3D 视图。

    # Add protein to the canvas
    view.addModel(pdb_block, 'pdb')  # 加载蛋白结构。
    ...
    if show_ligand:
        view.addModel(sdf_block, 'sdf')  # 加载配体结构。
...
def visualize_data(data, root, show_ligand=True, show_surface=True):
    protein_path = os.path.join(root, data.protein_filename)  # 构造蛋白文件路径。
    ligand_path = os.path.join(root, data.ligand_filename)  # 构造配体文件路径。
...
def visualize_generated_mol(protein_filename, mol, root, show_surface=False, opacity=0.5):
    protein_path = os.path.join(root, protein_filename)  # 构造蛋白文件路径。
    with open(protein_path, 'r') as f:
        pdb_block = f.read()

    view = py3Dmol.view()  # 创建新视图。

    # Add protein to the canvas
    view.addModel(pdb_block, 'pdb')
    view.setStyle({'model': -1}, {'cartoon': {'color': 'spectrum'}, 'line': {}})

    mblock = Chem.MolToMolBlock(mol)  # 将分子转换为 mol block。
...
def MolTo3DView(mol, size=(300, 300), style="stick", surface=False, opacity=0.5):
    ...
    viewer = py3Dmol.view(width=size[0], height=size[1])  # 设置视口大小。
